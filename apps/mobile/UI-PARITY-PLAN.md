...
- [x] 역 탭 메뉴를 웹 팝오버 스타일로 — 마커 위치 앵커드 팝오버(`popoverLayout`, 가장자리 클램핑·above/below 자동), 역할 버튼 세로 스택 + 역 상세
- [x] 경로 확인을 웹 `RouteConfirmDialog`와 동일한 모달로 — `RouteConfirmModal`(하단 슬라이드업 + 백드롭, 핀/경유/시작·취소)
**완료 기준:** 홈이 웹 홈과 상호작용까지 거의 동일. → tsc/lint 통과.

## Phase 3 — 노선 탭(웹 MapView) ✅
- [x] 웹 `MapView`와 동일 레이아웃: 세그먼트(지도/목록), 지도 뷰, 목록 뷰
- [x] 목록 뷰: 가로 스크롤 노선 선택칩(전체+노선) + `AllLines`(배지·이름·chevron) / `LineStations`(타임라인 점·연결선 + 환승 미니배지)
...
- [x] 섹션 구성: 알림·경로·데이터소스·**일반(신설)**·데이터관리·정보 + 언어/오프라인 데이터 "준비중" 행
- [x] 다크모드 토글 행 추가(Phase 9 에서 테마 컨텍스트와 연동 — 현재는 행 레이아웃/로컬 상태)
